...
inline void Internal::loc_search_assign (int lit, Clause * reason) {
  ...
  if (watching ()) {
    const Watches & ws = loc_watches (-lit);
    if (!ws.empty ()) {
      const Watch & w = ws[0];
      __builtin_prefetch (&w, 0, 1);
    }
...
bool Internal::propagate () {
  ...
  while (!conflict && propagated != trail.size ()) {
    ...
    propagate_all_local_watches();
    innerpropagate_with_normal_watches(lit);
  }
...
void Internal::innerpropagate_with_normal_watches(const int & lit) {
...
void Internal::propagate_all_local_watches() {
  // The level at which propagation of locally watched clauses needs
  // to start is different than the level of normally watched
  // clauses. It is reset in backtrack if necessary, and increased
  // here. In particular, after normal propagation of one level,
  // only new assigned literals are propagated with locally watched
  // clauses.
  while (!conflict && propagated_loc != trail.size ()) {
    const int lit = -trail[propagated_loc++];
    innerpropagate_with_local_watches(lit);
  }
}

void Internal::innerpropagate_with_local_watches(const int & lit) {
  LOG ("propagating %d", -lit);
    ...
    if (w.binary ()) {
      ...
      if (u > 0) j[-1].blit = other; // satisfied, just replace blit
        ...
        if (v > 0) {
          ...
          LOG (w.clause, "locally unwatch %d in", lit);

          lits[0] = other;
          lits[1] = r;
          *k = lit;

          loc_watch_literal (r, lit, w.clause);
	  ...
	  // The other watch is unassigned ('!u') and all other literals
	      ...
	      int pos, s = 0;
              ...
              lits[0] = other;
              lits[1] = s;
              loc_watch_literal (s, other, w.clause);
